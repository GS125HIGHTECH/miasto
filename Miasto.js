...
const geometry22 = new THREE.BoxGeometry( 0.1, 0.1, 0.1 );
...
const pointLight7 = new THREE.PointLight( 0xF8F8FF, 1, 1 );
  pointLight7.position.set( -9, -9, 6 );
...
// lights controls
  ...
  document.getElementById( 'point-light-środek' ).addEventListener(
    'click',
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
  ...
  document.getElementById( 'Camera-dom' ).addEventListener(
    'click',
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
        camera.position.z = camera.position.z==36 ? 7 : 36;
	    camera.position.x = camera.position.x==0 ? -9 : 0;
    },
    false
  );
