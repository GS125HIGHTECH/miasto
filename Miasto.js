...
const cubeTexture14 = new THREE.TextureLoader().load( 'textures/tire.jpg' );
  ...
  cubeTexture14.repeat.set( 5, 5 );

const geometry = new THREE.BoxGeometry( 10, 10, 0.1 );
const geometry2 = new THREE.BoxGeometry( 24, 24, 0.1 );
...
const geometry9 = new THREE.BoxGeometry( 11, 11, 0.1 );
...
const geometry14 = new THREE.CylinderGeometry( 0.04, 0.04, 0.7, 32 );
...
const material8 = new THREE.MeshStandardMaterial( {color: colors[14], map: cubeTexture4} );
...
const material17 = new THREE.MeshStandardMaterial( { color: 0xDB0707 } );
...
os.receiveShadow = true;

var os2 = os.clone();
os2.position.x+=1;

const podwozie = new THREE.Mesh ( geometry19, material15 );
podwozie.position.set(0.5, -8.9, 0.3);
podwozie.rotation.x-=Math.PI/2.0;
podwozie.receiveShadow = true;
...
var auto2 = auto.clone();
auto2.position.x+=5.4;

var auto3 = auto.clone();
auto3.position.x-=5.4;
material17.color.setRGB(0,1,0)

var auto4 = auto.clone();
auto4.rotation.y=Math.PI/2;
auto4.position.x=0.2;
auto4.position.z+=12;
...
// Lights and helpers
  ...
  spotLightHelper.color = 0xffad90;
  ...
  pointLightHelper.color = 0x8dff00;
...
// lights controls
  ...
  document.getElementById( 'spot-light' ).addEventListener(
    ...
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
      spotLight.intensity = spotLight.intensity==1 ? 0 : 1;
      renderer.render( scene, camera );
    },
    false
  );
  ...
  document.getElementById( 'point-light-domek' ).addEventListener(
    'click',
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
	  pointLight2.intensity = pointLight2.intensity==1 ? 0 : 1;
	  pointLight2_2.intensity = pointLight2_2.intensity==1 ? 0 : 1;
	  pointLight2_3.intensity = pointLight2_3.intensity==1 ? 0 : 1;
	  pointLight2_4.intensity = pointLight2_4.intensity==1 ? 0 : 1;
      renderer.render( scene, camera );
    },
    false
  );
  
  document.getElementById( 'point-light-boczne' ).addEventListener(
    'click',
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
      pointLight3.intensity = pointLight3.intensity==1 ? 0 : 1;
	  pointLight4.intensity = pointLight4.intensity==1 ? 0 : 1;
	  pointLight5.intensity = pointLight5.intensity==1 ? 0 : 1;
	  pointLight6.intensity = pointLight6.intensity==1 ? 0 : 1;
      renderer.render( scene, camera );
    },
    false
  );
  
  document.getElementById( 'spot-light-helper' ).addEventListener(
    'click',
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
      spotLightHelper.visible = spotLightHelper.visible==true ? false : true;
      spotLightHelper.update();
      renderer.render( scene, camera );      
    },
    false
  ...
  document.getElementById( 'point-light-helper' ).addEventListener(
    'click',
    function() {
      this.className = this.className.indexOf( 'inactive' ) > -1 ? 'active' : 'inactive';
      pointLightHelper.visible = pointLightHelper.visible==true ? false : true;
      pointLightHelper.update();
      renderer.render( scene, camera );      
    },
    false
    ...
    function(e) {
      ...
      switch ( e.key ) {
        ...
        case 'a':
		 ...
		 case 'c':
      	colorsCounter += 1;
      	if ( colorsCounter >= colorsNumber )
      		colorsCounter = 0;
      	material.color = new THREE.Color( colors[ colorsCounter ] );
      	break;
        default:
          ;
      }
